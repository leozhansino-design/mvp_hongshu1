...
3. **【极其重要】曲线趋势必须符合现实人生规律**：
   - score范围30-95，**早年分数必须偏低**（大多数人1-25岁还在成长学习阶段）
   - **绝大多数人的成就在30-50岁之间达到高峰**，不是一出生就高分！
   - 1岁评分范围：35-55分（童年无功绩可言）
   - 20岁评分范围：40-60分（刚步入社会，积累阶段）
   - 40岁评分范围：根据命局，50-85分（事业黄金期）
   - 60岁评分范围：根据命局，55-85分（收获期或衰退期）
   - 80岁评分范围：50-80分（晚年）
   - **典型的成功者曲线**：1岁45分→20岁50分→40岁75分→60岁82分→80岁70分
   - **普通人曲线**：1岁40分→20岁45分→40岁58分→60岁55分→80岁50分
   - **大器晚成型**：1岁42分→20岁48分→40岁55分→60岁78分→80岁75分
   - **禁止**：1岁就给60分以上，除非有特殊命理依据！
4. reason限10字内
...
2. **【极其重要】chartPoints曲线必须符合现实人生规律**：
   - 需80个点（1-80岁，每年一条），只需score（30-95），reason限15字
   - **早年分数必须偏低！**大多数人1-25岁还在成长学习阶段，没有什么功绩
   - **绝大多数人的成就在30-50岁之间达到高峰**
   - 1-20岁：35-55分区间（童年青少年期，无论命好命坏都是积累期）
   - 20-30岁：40-60分区间（步入社会，开始奋斗，但尚未成功）
   - 30-50岁：根据命局50-85分（事业黄金期，分数明显上升）
   - 50-70岁：根据命局55-90分（收获期或守成期）
   - 70-80岁：50-75分（晚年，精力衰退）
   - **典型成功者曲线**：1岁42分→10岁45分→20岁50分→30岁60分→40岁75分→50岁82分→60岁78分→70岁68分→80岁60分
   - **禁止**：1岁或20岁就给60分以上！这不符合现实人生规律！
   - 在每个10年大运内，允许有1-3分的小波动，但整体趋势要符合"低开高走"或"先抑后扬"
...
2. **chartPoints只需5个关键点**（1,20,40,60,80岁），daYun使用上面大运
3. **【极其重要】曲线必须符合现实人生规律**：
   - **即使是大格局命盘，早年也是积累期，分数不能高！**
   - 1岁：40-50分（童年无论如何没有功绩）
   - 20岁：45-55分（青年积累阶段，尚未发迹）
   - 40岁：65-85分（事业崛起期，分数开始明显上升）
   - 60岁：75-92分（事业巅峰或收获期）
   - 80岁：65-80分（晚年守成期）
   - **典型的大格局曲线**：1岁45分→20岁50分→40岁75分→60岁88分→80岁75分
   - **禁止**：1岁或20岁就给60分以上！30岁之前人生曲线应该偏低！
4. reason限10字内，用命理术语
