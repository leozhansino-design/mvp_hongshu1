...
3. **【最重要】chartPoints分数必须与summaryScore保持一致**：
   - ⚠️ chartPoints的分数必须围绕summaryScore上下波动！
   - 最高分 = summaryScore + 20（最多不超过95）
   - 最低分 = summaryScore - 20（最低不低于30）
   - 曲线的平均值应该接近summaryScore
   - 例如：summaryScore=62时，chartPoints应在42-82之间波动
   - 例如：summaryScore=75时，chartPoints应在55-95之间波动
   - ❌ 禁止：summaryScore=62却有chartPoints达到95或100！
4. **【曲线走势基于八字大运】**：
   - 曲线走势必须与大运吉凶完全对应
...
6. reason限10字内
7. **highlights和warnings各选1-2个**最关键的即可
8. 内容简洁，点到为止，避免过度详细`;
...
2. **【最重要】chartPoints分数必须与summaryScore保持一致**：
   - ⚠️ chartPoints的分数必须围绕summaryScore上下波动！
   - 最高分 = summaryScore + 20（最多不超过95）
   - 最低分 = summaryScore - 20（最低不低于30）
   - 曲线的平均值应该接近summaryScore
   - 例如：summaryScore=62时，chartPoints应在42-82之间波动
   - ❌ 禁止：summaryScore=62却有chartPoints达到95或100！
3. **【chartPoints曲线基于八字大运分析】**：
   - 需80个点（1-80岁，每年一条），只需score，reason限15字
...
4. **【数据一致性校验】**：
   ...
   - 【分数-词汇对照表】严格遵守：
     ...
     - score 50-64：可用"平淡、调整、起伏、考验"
     - score<50：必须用"低迷、受克、不利、坎坷、困顿、受阻"
5. daYunList使用上面提供的大运，添加详细description（80字）
...
4. **【数据一致性校验】**：
   - reason描述必须与score分数严格对应！
   - 高分词汇（score>=75）：巅峰、高光、大旺、得位、鼎盛
   - 低分词汇（score<50）：低迷、受克、不利、坎坷、困顿
   - **禁止**：score=40写"人生巅峰"这种自相矛盾！
5. reason限10字内，用命理术语
6. **所有描述只用命理术语**，禁止提及具体世俗事件
7. 整体分析要体现这是一个有潜力有格局的命盘`;
